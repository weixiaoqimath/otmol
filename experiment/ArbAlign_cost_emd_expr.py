...
def parse_molecule_pairs(file_path, mol_type):
    """
...
# Xiaoqi: here reflection is not not allowed, 
# however, ArbAlign itself includes a coordinate swaps and reflection step 
# (step 4 in the section 2.1 of the paper)
def kabsch(A, B): 
   """
...
"""
   ...
   for i in range(len(B_t)):
      ...
      #LAP = hungarian.lap(cost_matrix) # Xiaoqi: We just changed the hungarian algorithm to EMD
      EMD_P = ot.emd([], [], cost_matrix)
      if len(EMD_P) == len(np.unique(np.argmax(EMD_P, axis=1))):
         LAP = (np.argmax(EMD_P, axis=1), None)
      else:
         continue
      vars()[Perm[Uniq[l]]] = []
   ...
   if FinalRMSD < float(InitRMSD_unsorted): 
      ...
      FinalRMSD = float(rmsds[0][0]) # added by Xiaoqi
   else:
   ...
   write_to_xyz(num_atoms, './arbalign_emd_output/'+name, b_final_labels, b_final_coords)
   print "Best alignment of " + str(args.xyz2) + " with " + str(args.xyz1) + " is written to ./arbalign_emd_output/" + str(name)
   return FinalRMSD
...
if __name__ == "__main__":
   ...
   if True:
      ...
      pd.DataFrame(result).to_csv(('./arbalign_emd_output/cp_most_similar_result.csv'), index=False)
   ...
   if True:
      ...
      pd.DataFrame(result).to_csv(('./arbalign_emd_output/cp_most_different_result.csv'), index=False)
   ...
   if True:
      ...
      pd.DataFrame(result).to_csv(('./arbalign_emd_output/cp_largest_arbalign_result.csv'), index=False)
   ...
   if True:
      ...
      pd.DataFrame(result).to_csv(('./arbalign_emd_output/FGG_result.csv'), index=False)
   ...
   if True:
      ...
      pd.DataFrame(result).to_csv(('./arbalign_emd_output/S1MAW1_result.csv'), index=False)
   ...
   if True:
      ...
      pd.DataFrame(result).to_csv(('./arbalign_emd_output/ArbAlignDataWC_result.csv'), index=False)
   ...
   if True:
      ...
      pd.DataFrame(result).to_csv(('./arbalign_emd_output/1st2ndWC_result.csv'), index=False)
   ...
   if True:
      ...
      pd.DataFrame(result).to_csv(('./arbalign_emd_output/largest_RMSD_WC_result.csv'), index=False)
   ...
   if True:
      ...
      pd.DataFrame(result).to_csv(('./arbalign_emd_output/NeonCluster_result.csv'), index=False)
